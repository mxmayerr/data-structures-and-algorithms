...
public class TowersOfHanoi
{
  ...
  // background for red, yellow, green, blue, magenta, cyan, white high intensity
  ...
  /**
   * method to visualize the game state
   * precondition: gameState is populated with 3 lists of Integers
   * @throws IllegalArgumentException
   */
  public static void visualizeHanoiPegs() throws IllegalArgumentException
  {
    ...
    if (totalDiscs > 14)
    {
      throw new IllegalArgumentException("Too many discs for visualizeHanoiPegs(). Use showHanoiPegs() or add more colors instead.");
    }
    String[] colors = {RED_BACKGROUND, GREEN_BACKGROUND, YELLOW_BACKGROUND, BLUE_BACKGROUND, MAGENTA_BACKGROUND, CYAN_BACKGROUND, WHITE_BACKGROUND, RED_BACKGROUND_BRIGHT, GREEN_BACKGROUND_BRIGHT, YELLOW_BACKGROUND_BRIGHT, BLUE_BACKGROUND_BRIGHT, MAGENTA_BACKGROUND_BRIGHT, CYAN_BACKGROUND_BRIGHT, WHITE_BACKGROUND_BRIGHT};

    // create a string array to hold the visualization
    String[] pegs = new String[totalDiscs];
    // to avoid null pointer exceptions, fill pegs with empty strings
    Arrays.fill(pegs, "");

    // create copy of gameState
    ArrayList<ArrayList<Integer>> gameStateCopy = new ArrayList<>();
    for(int i = 0; i < gameState.size(); i++)
    {
      gameStateCopy.add(new ArrayList<Integer>());
    }
    for(int i = 0; i < gameStateCopy.size(); i++)
    {
      for(int j = 0; j < gameState.get(i).size(); j++)
      {
        gameStateCopy.get(i).add(gameState.get(i).get(j));
      }
    }

    // for the lists with empty values (no discs), fill it in with -1
    for(int i = 0; i < 3; i++)
    {
      while(gameStateCopy.get(i).size() < totalDiscs)
      {
        gameStateCopy.get(i).add(-1);
      }
    }
    // now we know that each list has the same number of elements, just makes it easier later on

    // calculate the width of each column
    int width = 2 * totalDiscs + 1;

    // we do a for loop for each row and add the result to the string array
    // for each disc we need to print
    for (int i = 0; i < totalDiscs; i++)
    {
      ...
      {
        // get the disc number
        int discNum = gameStateCopy.get(j).get(i);

        // if the disc number is -1, then there is no disc
        if(discNum == -1)
        {
          // add the empty peg to the string array
          pegs[i] += " " + " ".repeat(width - 2) + " ";
        }
        else
        {
          ...
          // add the disc to the string array
          pegs[i] += " ".repeat((width - discSize) / 2) + colors[discNum] + " ".repeat(discSize/2) + discNum + " ".repeat(discSize/2) + RESET + " ".repeat((width - discSize) / 2);
        }
        pegs[i] += "  ";
      }
    }

    // print the pegs from bottom to top
    for(int i = pegs.length - 1; i >= 0; i--)
    {
      System.out.println(pegs[i]);
    }
    // print the peg labels (A, B, C)
    System.out.println("  " + " ".repeat(width/2) + "A" + " ".repeat(width) + "B" + " ".repeat(width) + "C" + " ".repeat(width/2));
